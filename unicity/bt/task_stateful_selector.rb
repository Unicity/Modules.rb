...
module Unicity
	...
	module BT
		...
		class TaskStatefulSelector < Unicity::BT::TaskSelector
			...
			def process(exchange)
				...
				while @state < @tasks.count
					...
					if [Unicity::BT::Task::Status::SUCCESS, Unicity::BT::Task::Status::ERROR, Unicity::BT::Task::Status::QUIT].include?(status)
						@state = 0
						return status
					elsif status == Unicity::BT::Task::Status::ACTIVE
						return status
					elsif status == Unicity::BT::Task::Status::INACTIVE
						 inactives += 1
					end
					@state += 1
				end
				...
				if inactives < @tasks.count
					return Unicity::BT::Task::Status::FAILED
				else
