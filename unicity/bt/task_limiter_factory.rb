...
require "./task_limiter.rb"

module Unicity

	module BT

		class TaskLimiterFactory

			def getObject(parser, element)
				type = !element["type"].nil? ? element["type"] : "Unicity::BT::TaskLimiter"
				...
				if !object.is_a?(Unicity::BT::TaskLimiter)
					raise Exception.new("Invalid type defined. Expected a task limiter, but got an element of type \"#{type}\" instead.")
				end
